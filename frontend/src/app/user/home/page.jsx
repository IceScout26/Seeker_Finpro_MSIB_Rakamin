...
export default async function UserHome() {
  ...
  return (
    <div className='bg-white'>
      <section className='h-[calc(100vh-80px)] w-screen p-8 flex flex-col items-center justify-center bg-cover bg-right' style={{ backgroundImage: `url('../../assets/LP/hero-bg.jpg')` }}>
        <div className='w-4/6 h-60 border border-white rounded-md text-white text-center py-4 px-10 mt-44'>
          {/* <h2 className='font-semibold text-xl'>Job Seeker 2023</h2> */}
          <h1 className='text-3xl font-semibold'>Seek your dream job easily with SEEKER.</h1>
          <h3 className='text-xl mt-4'>Explore thousands of latest job openings across various industries directly from our front page, and kickstart your career journey today</h3>
        </div>
        <div className='flex justify-between items-center w-3/6 h-32 -translate-y-16 bg-white rounded-md text-center py-4 px-10'>
          ...
          <div>
            <span className='font-semibold text-2xl'>+400</span> <br /> Companys
          </div>
          <div>
      ...
      <section className='container mx-auto text-center py-32'>
        <h2 className='text-xl'>Dapatkan Kesempatan Dilirik oleh Perusahaan ternama <br /> dengan membangun personal branding terbaikmu</h2>
        <div className='flex gap-8 justify-center items-center mt-8'>
          {
            companyLogos.map((logo, id) => {
              return <div className='border border-gray-200 rounded-md shadow-sm bg-white h-24 w-48 flex justify-center items-center'>
                <Image src={logo.logo} key={id} alt="" srcSet="" className={logo.width + ' ' + logo.height} />
              </div>
            })
          }
        </div>
      </section>
      <section className='bg-yellow-800 bg-opacity-20 w-full flex justify-center items-center py-32'>
        <div className='container mx-auto'>
          <div className='h-80 w-full flex justify-between items-center p-10 shadow-md rounded-md bg-white'>
            <div className='w-1/5 flex flex-col gap-2'>
              <h2 className='text-2xl font-semibold'>Dukungan & Garansi Purna Jual</h2>
              <span className='text-sm'>Komitmen DomaiNesia mulai dari pertanyaan pra penjualan hingga dukungan purna jual untuk layanan cloud hosting Indonesia.</span>
            </div>
            <div className='w-1/4 flex flex-col gap-2 p-4 border-2 border-l-0 border-t-0 border-b-0 border-r-yellow-600'>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor" className="w-16 h-16 text-yellow-800">
                <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z" />
              </svg>
              <span>Bantuan TimSupport 24/7</span>
              <span className='text-sm'>Anda berhak mendapat pelayanan terbaik dari seluruh tim DomaiNesia, kami selalu siap 24/7 untuk membantu Anda.</span>
            </div>
            <div className='w-1/4 flex flex-col gap-2 p-4 border-2 border-l-0 border-t-0 border-b-0 border-r-yellow-600'>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor" className="w-16 h-16 text-yellow-800">
      ...
      <section className='container mx-auto py-32 px-5'>
        <div className='text-center'>
          ...
          <p className='mt-2'>Kembangkan Keterampilan Anda dengan Mentor Ahli dan Proyek Pekerjaan Nyata!</p>
        </div>
        <div className='grid grid-cols-2 gap-4 mt-8'>
        {
          programs.map((program, id) => {
            return <div key={id} className='flex gap-4 items-start justify-start'>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor" className="w-32 text-yellow-800">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
            </svg>
        ...
        <div className='flex gap-4 flex-wrap justify-center'>
          {
            members.map((member, id) => {
              return <div key={id} className='w-72 flex flex-col items-center justify-center'>
                <Image src={avatar} className='w-20 h-20 rounded-full translate-y-10' />
