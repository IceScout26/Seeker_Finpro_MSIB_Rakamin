...
export default function userExplore() {
  ...
  async function getUserExplore(token) {
    ...
    if (token.date === today) {
      const API = await fetch("http://localhost:5000/jobuser/user", {
        method: "GET",
        headers: {
  ...
  return (
    <div className="container h-full w-screen bg-white pb-16">
      ...
      <div className="grid grid-cols-3 place-items-center">
        {exploreData.map((explore) => {
          return <div className="w-96 row p-9 text-brown-gerry rounded-lg shadow-lg border border-gray-200 cursor-pointer mr-4">
