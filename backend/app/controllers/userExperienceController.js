...
const getExperienceByIdController = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).send("An error occurred while fetching experience.");
  }
};
...
const createExperienceController = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).send("An error occurred while creating experience.");
  }
};
...
const updateExperienceByIdController = async (req, res) => {
  ...
  try {
    const experience = await updateExperienceById(
      experienceId,
      updatedExperience
    );
    res.status(200).json(experience);
    ...
    console.error(error);
    res.status(500).send("An error occurred while updating experience.");
  }
};
...
const deleteExperienceByIdController = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).send("An error occurred while deleting experience.");
  }
};
